...
- 小题请**先看"采分点共识表"（共识点／单源点／材料推定点／争议点），再看分数**——
  它说明"这个点为什么算分、那个点为什么只是低权"（L3 模拟口径，见《评分口径》一节）。
...
（写成 `材料X第Y段"……"`），**拿不出就删除**，不进采分点表；② **共识度不能排除材料确凿的点**——
参考答案是校准尺、不是判据，机构集体漏点是常态。
...
**分值池纪律**：单源点 ＋ 材料推定点合计 **≤30%**（共识点合计占 **70%–80%**）；
**漏掉它们只扣该点小分值，不得据此判定"内容不全面"或整体降档**。
**无共识点**（无参考答案／只有 1 家／全同源）→ 整题退回**纯模拟重构（L3）**，
全部按"**材料推定点**"，报告写明"本题无共识点，采分点为模拟重构"，**置信度中／低**。
...
- **两条硬纪律**：**① 题面许可原则**——题面**没写**"见解深刻／观点独到"时，**不得以"不够深刻"为由
  降档或扣题意分**（只能用"观点明确、准确"这一档要求）；**② 点名层次原则**——说"不够深刻"**必须
...
**范文不是一类文：只作相对刻度（L3 模拟口径 ＋ L4 经验校准，非官方）**
...
- **梯队**：**T1**（最高水平）／**T2**（中等、合格线附近）／**T3**（确有才用）；
  **每梯队 ≥2 篇才成刻度**（单篇不成刻度），且 **T1 与 T2 都要有**。来源优先级：
...
- **怎么用**：先按四维**独立定档**（刻度**不参与定档**）→ 再读画像做**六项对照** →
  结论**只用于档内定位与改进方向**；**禁止反推分数**、**禁止因"像范文"加分**。
- **可提炼该卷大作文批改规范**：只提炼**立意梯度／核心论点池（3–5 个）／常见失分点／硬性与禁止口径**
  四样，**不提炼分数分界**（一类／二类的分数分界要有反例样本才能提）；落盘
...
- **缓存与 token 纪律**：`批改报告/.大作文刻度/` 下存 `{卷ID}-素材清单.md`（来源指纹＝
  ...
  **日常批改只读梯队画像 ＋ 该卷规范（约 500–1k tokens）**，**禁止每次重读范文原文**；
  没有刻度集时按常规四步正常批改，只是不给"刻度参照"一段，**不得因此降低批改完整度**。
- **何时建刻度**：同一题反复写（≥2 稿）、重点备考卷、已有 ≥2 篇同梯队样本才值得建；
  **只有单篇或只批一次 → 不要建**。
