...
- [评分口径：这套 skill 到底怎么打分](#评分口径这套-skill-到底怎么打分)
  ...
  - [字数：核验归核验，扣分归扣分](#字数核验归核验扣分归扣分2026-09-19-修订)
  - [输出与落盘](#输出与落盘)
...
9.19修改：受到申论阅卷的启发，申论阅卷是先抽取部分试卷进行试批改，然后才得出批改规范。所以如果你上传了某套题的参考答案，在进行批改之前，它会根据参考答案建立采分点，避免出现生成的采分点和所有机构不同的情况。
关于大作文，我并不认为机构的作文一定是一类文，有的老师也承认自己的作文是二类文，ds也经常将范文划为二类（这也和我的反模板原则有关），你可以手动对你认为好的作文进行标注，或者增加规则（如粉笔的作文强制定为一类文），目前会根据范文建立评分细则。

不过我本人能力和经济都有限，未对该skill做完整测试，未来还会做更多修改。或许它在不同模型（fable 5、gpt 6、kimi k3、glm 5.3）表现会更好，而且很可能通过这些模型能够修改更完善，不过我没有实力去尝试。但我的目的是搭建一个框架，你可以按你的方式继续完善。
...
内容含**评分依据声明（L1–L4 ＋ 置信度）→ 逐题采分点核对（附原文证据）→ 建议区间与置信度 →
字数核验块（示例答案／范文给字数，你的作答只给"符合／超出上限／明显偏少"的定性结论）→ 改进方向排序**。
...
6. **试批改标定（阶段 A，必做）**：拿本题参考答案**自己试批改一遍**（逐份核对
   ...
   （其中"**脚本不可用（未做同源检测）**"因家数未知、资料可判性不足，**降为"低"**）。
...
**大作文的阶段 A 是"范文刻度"**（见下文"范文不是一类文"）：**已建刻度／仅 1 篇记"单篇参照"／
无范文但已用 ≥2 家参考答案试批改**，三者之一才算"已完成阶段 A"；
**"无范文、不建刻度 ＋ 未做试批改"不算完成**（按未完成处理，**置信度上限"中"**）。
...
**"0 家＝0 分"的三层口径（2026-09-19 修订，消除"到底 0 分还是低权"的两套答案）**：
...
> **"参考资料的唯一例外"只针对后两种情形**（资料残缺／家数未知）；
> **"恰好 1 家／无参考答案／全同源"不等于"资料残缺"**：家数已判明时走"无共识点"路径，
> **低权计分但置信度为"中"**（见本节"评分置信度"）。
...
**冲突时的裁决顺序（低分题必用，2026-09-19 修订）**：
**① 分值池合计＝题目总分 → ② 单点 10%–35% → ③ 共识点 > 单源点（是"排序"不是占比）
→ ④ 目标区（70%–80%／20%–30%、40% 上限）**，④ 与 ①②③ 冲突时**一律让位**。
**10–15 分题不适用 40% 硬上限**（单源点可以低于总分 10%，不算破例）。
**点数可行区间 3 ≤ N ≤ 10**（由"每点 ≥总分×10%"与"每点 ≤总分×35%"共同限定；
如 **10 分题 3 点或 4 点都可行**——4 点＝每点 2.5 分＝25%）。

**共识点不足 3 个时（2026-09-19 修订）**：先合并同类要点；合并后仍 <3 个 →
**允许点数少于常规、按实际点数均分题目总分**（此时 35% 上限不适用），
**且该题置信度降为"低"**；**若该题仍有单源点，则单源点全部转入提醒区、不计分**
（否则"共识点合计＝总分"与"单源点 ≤40%"在数学上无解）；
**"整题无共识点"是另一条路径**（见下）——两者不要混。
**例外**（参考资料**残缺或家数未知**／未做同源检测）：提醒项按低权计分并进入计分表，
此时低权池 ＝ 单源点 ＋ 材料推定点，**不受 20%–30% 目标区与 40% 上限约束**。
**无共识点**（无参考答案／只有 1 家／全同源／脚本不可用）→ 整题退回**模拟重构（L3）**，
点全部按低权计分（不受 40% 约束），报告写明"本题无共识点，采分点为模拟重构（含低权保留）"；
**置信度：家数已判明（0 家／1 家／全同源）＝中；家数未知或资料残缺＝低**。
...
3. **第三步｜档内定位（上／中／下三小档）**：大档锚定后**取消"±1／±0.5 机械加减"**，
   改为在本大档内分**上、中、下三个小档**（1/3、2/3 点**就近取到 0.5 刻度**——
   34.67→34.5、29.83→30、32.67→32.5、33.17→33——**居中取大**），
   按**①题意质量、②内容质量、③论证深度、④结构完成度、⑤语言质量五项**的实际表现
   **取该小档的落点值**（**写落点值，不写区间**）；
   **边界值 0.5 归上一小档**（如二类 29 属"上"、26.5 属"中"），
   落分**不得越出本大档区间**（想越档是第一步的事，不是第三步的调节手段）。
   **亮点不单独加分**（不设"创新分"）：够上一档的水准，就在第一步把基础档位定上去。
4. **第四步｜区间与置信度（必给）**：主分之外必须给**建议区间**＋**评分置信度（高/中/低 + 理由）**；
   **窄区间以本小档落点值为界**（如 40 分卷二类中＝**26.5–28.5**；需跨小档时只给档位不给落点）；
...
> 先用四维综合**锚定大档**，再按**五项**（①题意质量②内容质量③论证深度④结构完成度⑤语言质量）
> 的实际表现**定位到上／中／下**（**边界值归上一小档**）。
> **本表用简写**：`下 a–b` ＝ **从 a 到 b 全部可落**（等价于 `[a, b+0.5）`），
> 如二类"下 24–26"＝24／24.5／25／25.5／26、**"中"从 26.5 起**；
> **权威形式（半开闭区间 ＋ 逐值枚举）见 `references/scoring-standards.md` 三.1**。
...
- **高** ＝ 材料与作答完整 ＋ 该卷**有批改规范**，或有**来源独立、覆盖本题**的参考答案可作校准；
  **小题**还须该题**有强/弱共识点**，且**已完成阶段 A 试批改**；
- **中** ＝ 材料与作答完整，但可校准口径缺失或只覆盖部分（只有单一起源的答案、答案与本年卷不符、
  只有小题口径没有大作文口径），档位边界无法交叉验证；
- **低** ＝ 材料不全／作答由图片转写且有存疑／题干或分值缺失；
- **分层口径（2026-09-19 修订，这一条最容易被读错）**：
  **"仅 1 家独立来源／无参考答案／全同源"＝中**（家数**已判明**、资料本身完整，只是校准口径单一或缺失）；
  **"未做同源检测（家数未知）／脚本不可用"或"疑为节选、材料残缺"＝低**。
  另有 **"共识点不足 3 个而按实际点数均分"** 这一小题情形，也判**低**。
- **份数多 ≠ 置信度高**：**同一来源的多份只算一份**；不得因"参考答案份数多"就判"高"，
  也不得因"没有官方标准"而拒评（依据不足就给宽区间 ＋ 低置信度 ＋ 说明缺什么）。
- **大作文判"高"的阶段 A 前置**：已建刻度（T1 单向／双向）／仅 1 篇范文已记"单篇参照"／
  无范文但已用 ≥2 家参考答案试批改——三者之一；**"无范文、不建刻度 ＋ 未做试批改"不算完成阶段 A**，
  按未完成处理（**上限"中"**，报告不得写"阶段 A 已完成"）。

### 字数：核验归核验，扣分归扣分（2026-09-19 修订）

- **评分不看字数**：真实阅卷只看答题卡格子占用与内容完整度，"X 字左右"是弹性标准；
  字数问题只通过"内容单薄／结构没收束"等维度体现，**不机械倒扣**。
- **但字数要求是 L1，必须核验合规性**（`工具/count_chars.py`，答题卡口径：含标点，不含空格换行）：
  - **用户作答**：只给**定性结论**——"已核验：符合题面字数要求"／"**超出题面上限**"／"明显少于要求"；
    超限时在报告里**单列一行"⚠ L1 合规问题：字数超出题面上限"**，并写明
    "**官方罚则未公开，本 skill 不虚构扣分公式、不据此扣分**"。
    **报告里不出现"用户作答 X 字"这类具体数字**；明显偏少或**被截断**时按**结构完成度／内容质量**如实反映。
  - **示例答案／参考范文**：目标区按题干表述查 `word-count.md` §3（"不超过 N 字"类≈**上限的
    85%–95%**；"X 字左右"≈X±10%；"1000—1200 字"落在区间内），且**必须 ≤ 上限**；
    生成后**必须跑脚本核验**，未核验不得标注"（约 X 字）"；大作文写成"含标题 X 字（正文 Y 字）"。
- **"必须核验"≠"必须扣分"**：L1 要求真实存在所以不能不查；官方从未公布超字罚则所以不能编扣分公式。
...
报告必备：**评分依据声明（L1–L4＋置信度）、逐题采分核对与原文证据、建议区间与置信度、
字数核验块（示例答案／范文给数字；用户作答只给"符合／超出上限／明显偏少"的定性结论）、改进方向排序**。
...
4. **字数：核验是硬要求，扣分公式不存在**——示例答案／范文必须跑字数核验
   （目标区按题干表述查 `word-count.md` §3，"不超过 N 字"类≈上限 85%–95%），
   没条件跑脚本时明确标注"字数为估算"；**用户作答的题干字数要求（L1）须核验合规性**，
   超限在报告单列"L1 合规问题"，但**不得据此扣分或降档**。
...
4. **"0 家不计分"是经验校准假设，不是判据**：材料确凿但机构参考答案集体未收录的点，
   本 skill 的处理是**进提醒区、不计分**（资料完整且有 ≥2 家独立来源时）——
   这是**为了提高采分点可靠性而设的过滤机制**，**不代表真实阅卷中该点必然不给分**。
   更根本的是：**"机构集体漏点"与"该点确实不是采分点"，在真实阅卷中无法区分**（见
   `references/scoring-standards.md` 0.3），所以这条规则只保证"不让你为专业答案都没写的点丢分"，
   不保证与实际采分表一致。**参考答案始终只是校准尺（L4），不构成对错判据。**
5. **公文权重无官方依据**：五项检查的先后顺序有道理，但"内容／格式／语言"到底各占多少分，**没有官方比例**。
6. **大作文的"深刻""充实"仍是判断**：四维描述可以对齐尺度，但"够不够深刻"最终靠阅卷人式的判断，
   不同判断者之间仍会有 ±2 分左右的漂移（这也是建议区间存在的原因）。
7. **反模板做得激进**：skill 明确"类型/结构不参与打分"，好处是不会因为不写五段三分被冤扣，
   代价是**对"结构极漂亮但内容空"的文章给分可能偏软**（依赖内容维度扣分来平衡）。
8. **刻度参与定档本身是循环校准**：T1／T2 梯队的"参照档"是**同一个模型用同一套四维规则判出来的**
   （L4 推断），所以它**不是独立于四维的第二套证据**，只能当"相对标尺"用；
   四维判定与刻度冲突时以**题面（L1）＋四维实际表现**为准（见 `essay-exemplars.md` 第 4 节末）。

**工程与使用层面**

9. **依赖人工投喂**：`作答记录/`、`参考答案/` 与 `批改规范/` 都得你自己放；
   覆盖率不足的年份与卷型只能靠纯模拟重构，档位边界无法交叉验证（置信度上不去）。
10. **PDF 提取有坑**：部分真题 PDF（尤其带机构水印的）文字层可能乱码，需要换提取方式或 OCR，
    并人工校对；OCR 稿一律标注"转写稿经用户校对"。
11. **字数脚本只做字数**：`count_chars.py` 用答题卡口径（含标点、不含空格换行），
    与 Word 字数统计、纯汉字数都不完全一致——报告统一以脚本值为准，但**不要拿去和别的工具对比数字**。
12. **不会替你写整篇**：默认只给方向、采分点、示例答案（小题）与改进建议；要整篇范文得明确说。
13. **置信度是自评**：高/中/低的判定条件（材料与作答是否完整、有无该卷批改规范、有无**来源独立且
    覆盖本题**的参考答案）是 skill 自定的**"采样与校准"口径**，**不随参考答案份数上升**
    （同源多份只算一份）；它衡量的是"这次批改有多可靠"，不是"你的分数有多准"。
14. **只覆盖申论**：只处理申论批改，行测与面试类内容不在处理范围。
15. **真题库已外置到工作区**：题库不再随 skill 分发，而是放在 `真题/{省份}/真题库/`
    （`INDEX.md` ＋ `{卷ID}.md`，模板 `真题/_TEMPLATE.md`）；**新环境默认没有题库**，
    "真题识别"以及库内题面／评分依据的匹配能力，**取决于你是否自行入库**。
16. **`批改规范/` 依赖用户提供**：官方阅卷细则通常不公开，规范得靠你自己收集；
